...
class MetaModel:
    ...
    def _fx_couple(self, state, dt, ts, coupler):
        ...
        # if ts // (self.m1.dt // self.m2.dt) == 0:

        state_couple = self._metamodel_update_X(predicted_m1, predicted_m2, coupler, ts)

        # else:

        #     state_couple = np.array(list(state[:self.m1.n_state]) + list(predicted_m2))
    ...
    def inference(self, n_repeats, verbose=0):
        ...
        for i in range(n_repeats):
            ...
            ukf_meta = UKF(dim_x=self.n_state, dim_z=self.n_state, fx=self._fx_couple, hx=self._hx, dt=min(self.m1.dt, self.m2.dt), points=sigmas_meta)
            ...
            for ts in range(n_step):
                ...
                ukf_meta.predict(fx=self._fx_couple, dt=min(self.m1.dt, self.m2.dt), coupler=self.coupling_graph.coupling_variable[0][ts, 0], ts=ts)
