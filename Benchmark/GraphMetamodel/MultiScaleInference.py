...
class MetaModel:
    ...
    def inference_for_ts(self, x_ts, P_ts, ts, omega_ts):
        ...
        sigmas = MerweScaledSigmaPoints(self.meta_n_state, alpha=1e-3, beta=2., kappa=0.)
    ...
    def inference(self, test_omega, filepath=None, verbose=1):
        ...
        for ts in range(self.max_n_step):
            ...
            # omega_ts = [self.coupling_graph.omega[ts], self.coupling_graph.omega[ts]] 
            omega_ts = test_omega
            # omega_ts = [self.coupling_graph.omega[ts][0], self.coupling_graph.omega[ts][1]]   
            # omega_ts = 0.5    
            # print(omega_ts)   
