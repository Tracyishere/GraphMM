...
class MetaModel:
    ...
    def __init__(self, coupling_graph):
        ...
        self.coupler = self.coupling_graph.coupling_variable
        ...
        self.R = [np.diag(list(np.diag(self.m1.R[i]))+list(np.diag(self.m2.R[i]))) for i in range(self.m1.n_step)]
        self.Q = [np.diag(list(np.diag(self.m1.Q[i]))+list(np.diag(self.m2.Q[i]))) for i in range(self.m1.n_step)]
    ...
    def _metamodel_update_X(self, m1, m2, coupler, ts):
        ...
        couple_idx_m1, couple_idx_m2 = self.coupling_graph.connect_idx
        epsilon = self.coupling_graph.epsilon
        omega_1 = self.coupling_graph.omega_1; omega_2 = self.coupling_graph.omega_2
        phi_1 = self.coupling_graph.phi_1; phi_2 = self.coupling_graph.phi_2
    ...
    def _metamodel_update_P(self, ts):
        ...
        omega_1 = self.coupling_graph.omega_1; omega_2 = self.coupling_graph.omega_2
        phi_1 = self.coupling_graph.phi_1; phi_2 = self.coupling_graph.phi_2
        phi_1_ts = phi_1[ts][couple_idx_m1]
        phi_2_ts = phi_2[ts][couple_idx_m2]
        P_meta = np.diag(list(np.diag(self.m1.Q[ts]))+list(np.diag(self.m2.Q[ts])))
    ...
    def inference(self, n_repeats, verbose=0):
        ...
        for i in range(n_repeats):
            ...
            obs_m1 = np.random.normal(loc=self.m1.obs[:,:,0], scale=self.m1.obs[:,:,1])
            obs_m2 = np.random.normal(loc=self.m2.obs[:,:,0], scale=self.m2.obs[:,:,1])
            ...
            for ts in range(n_step):
                ...
                ukf_meta.predict(fx=self._fx_couple, dt=self.m1.dt, coupler=self.coupling_graph.coupling_variable[ts, 0], ts=ts)
