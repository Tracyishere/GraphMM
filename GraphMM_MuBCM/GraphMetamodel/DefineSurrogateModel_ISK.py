...
class SurrogateInputModel:
    ...
    def get_obs_ts(self, x0, time, Vm):
        
        obs_t_interval = get_observations_fx_with_one_step(x0, self.fx, self.dt, time, self.measure_std_scale, Vm)
        obs_temp = np.random.normal(loc=obs_t_interval[:, :,0], scale=obs_t_interval[:, :,1])
                
        return obs_temp[0]


    def inference_for_ts(self, x_ts, P_ts, time, Vm):
        ...
        obs_ts = self.get_obs_ts(x_ts, time, Vm)
        surrogate_ts.R = self.get_R_ts(obs_ts)[0]
        # print(np.diag(surrogate_ts.R))
        if Vm is not None:
            surrogate_ts.predict(dt=self.dt, fx=self.fx, step=time, V_list=Vm)
        else:
            surrogate_ts.predict(dt=self.dt, fx=self.fx, step=time)
        surrogate_ts.update(obs_ts)

        return surrogate_ts, obs_ts


    def inference(self, Vm=None, obs_filepath=None, output_filepath=None, n_repeat=1, verbose=1):    
        ...
        for k in range(n_repeat): 
            # repeat for different observations. not support for ISK model since the RAM is out of momery
            ...
            for t in range(self.n_step):
                ...
                surrogate_ts, obs_ts = self.inference_for_ts(x_ts, P_ts, t, Vm)
                x_ts = surrogate_ts.x
                P_ts = surrogate_ts.P
